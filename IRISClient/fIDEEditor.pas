...
uses
  ...
  fLibraryBrowser,
  {$ENDIF}
  ToolWin, Tabs, ImgList, ActnList, System.Actions;
...
type
  ...
  public
    constructor Create(AOwner: TComponent; ShowInTaskBar: Boolean); reintroduce;
...
constructor TIDEEditorForm.Create(AOwner: TComponent;
...
procedure TIDEEditorForm.CreateParams(var Params: TCreateParams);
begin
...
begin
  ...
  acViewLibraryBrowser.Visible := True;
  {$ELSE}
  ...
  ControlBar1.DrawingStyle := ExtCtrls.dsNormal;
  {$ENDIF}
  //IDEMemo1.BorderStyle := bsNone;
  ...
  for c := 0 to ControlBar1.ControlCount - 1 do
    ...
    begin
      {$IFDEF DELPHI2006_LVL}
      DrawingStyle := ComCtrls.dsNormal;
      {$ENDIF}
      EdgeInner := esNone;
      EdgeOuter := esNone;
      Transparent := true;
...
begin
  ...
  if Assigned(FOldOnRunningChanged) then
    FOldOnRunningChanged(Sender, ARunning);
end;
...
procedure TIDEEditorForm.ControlBar1BandPaint(Sender: TObject;
  Control: TControl; Canvas: TCanvas; var ARect: TRect;
  var Options: TBandPaintOptions);
begin
  Options := Options - [bpoFrame];
end;
